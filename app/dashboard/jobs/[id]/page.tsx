...
function MockPaperContent({ part = 1, vertical = true }: { part?: number, vertical?: boolean }) {
  // Styles to simulate a document layout
  return (
    <div className={`w-full h-full flex ${vertical ? 'flex-col space-y-1' : 'flex-row space-x-1'} opacity-50`}>
      {/* Simulate a Header Image on Part 1 */}
      {part === 1 && <div className={`${vertical ? 'h-8 w-full' : 'w-8 h-full'} bg-gray-200 rounded-sm`}></div>}
      
      {/* Simulate Text Lines */}
      <div className="flex-1 space-y-1">
        <div className={`bg-gray-200 rounded-sm ${vertical ? 'h-1 w-full' : 'w-1 h-full'}`}></div>
        <div className={`bg-gray-200 rounded-sm ${vertical ? 'h-1 w-5/6' : 'w-1 h-5/6'}`}></div>
        <div className={`bg-gray-200 rounded-sm ${vertical ? 'h-1 w-full' : 'w-1 h-full'}`}></div>
        {part === 2 && (
          <>
             <div className={`bg-gray-200 rounded-sm ${vertical ? 'h-1 w-full' : 'w-1 h-full'}`}></div>
             <div className={`bg-gray-200 rounded-sm ${vertical ? 'h-1 w-4/6' : 'w-1 h-4/6'}`}></div>
          </>
        )}
      </div>
